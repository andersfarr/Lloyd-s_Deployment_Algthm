function [delta_x, delta_y] = velocity_fun(Velocity_Type,direction_Vector,velocity,max_velocity,MOVEMENTSCALE,algorithm_type, Agent_Positions, agent_number, blowerX, blowerY)
% Velocity Types: 
...
elseif algorithm_type == 2
    %%%%%%%%%%%%%% Your velocity type here!%%%%%%%%%%%%%%%%
       dist_to_blower = (sqrt((Agent_Positions(agent_number,1)-blowerX)^2+(Agent_Positions(agent_number,2)-blowerY)^2));
       delta_x = 5*(60/(2*pi))*direction_Vector(1)*dist_to_blower;
       delta_y = 5*(60/(2*pi))*direction_Vector(2)*dist_to_blower;
    end
end